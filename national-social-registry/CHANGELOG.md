...
## national-social-registry — develop 0.0.0-develop.162 (2026-07-23)

_commit `e992e79` · baseline: release the start · previous build 0.0.0-develop.159_
<!-- build:0.0.0-develop.162 revision:e992e797a1e7dcc5eafb55feb41c894f2c028ceb ts:1784798430 -->

### Summary

_All changes since release the start:_

- **Major:** Refactored NSR as a thin extension, replacing the self-sufficient chart with an openg2p-nsr wrapper and dropping inherited db-seed machinery; updated Docker images to be thin and aligned with platform images.
- Database updates: Restored NSR's load_sample_data.py and upload_images.py, fixed SQL insert statements, and enhanced geo data loading functionality; added new fields and improved data consistency across various seed data scripts.
- CI/CD improvements: Integrated NSR field-specific test set into CI, updated Helm chart configurations, and streamlined Dockerfile for better clarity and functionality.
- Security enhancements: Implemented CSRF validation for staff-portal-api requests, added IAM public URL configuration, and ensured dockers run as non-root users to improve production hardening.
- Dependency management: Bumped dependencies to latest versions, frozen AWE and postgres-init versions, and updated image tags across multiple Docker components.
- Feature additions: Introduced pre-approval logic for change requests, added G2PScoreComputeServicePoverty for poverty score computation, and enhanced JSON templates for DCI response handling.
- Cleanup and refactoring: Removed legacy logging, hardcoded values, and unnecessary permissions; improved validation functions and streamlined search text handling across services.

### New in this build (since 0.0.0-develop.159)
...
## national-social-registry — develop 0.0.0-develop.159 (2026-07-23)

_commit `d086d55` · baseline: release the start · previous build 0.0.0-develop.158_
<!-- build:0.0.0-develop.159 revision:d086d55cf164849e2ee2ca92df69c0f90f0b43b7 ts:1784796303 -->

### Summary

_All changes since release the start:_

- **Major:** NSR refactor: transitioned to a thin extension model, replacing the self-sufficient chart with an openg2p-nsr wrapper, and updated Docker images to use platform base images.
- CI enhancements: integrated NSR field-specific test set into CI, added openg2p-nsr-sanity-tests image, and aligned version pins with chart dependencies.
- Security improvements: implemented CSRF validation for staff-portal-api requests and ensured Docker containers run as non-root users.
- Database updates: enhanced db-seed functionality with geo data loading, updated SQL seed data for households and individuals, and improved error handling in data processing services.
- Dependency management: updated various dependencies and Docker components to the latest versions, including a frozen version for postgres-init.
- IAM and permissions: synchronized operational role IAM permissions for register metadata API access and updated permission-to-role mappings.
- Documentation and cleanup: added README for NSR extension package, moved documentation to Gitbook, and removed legacy logging and hardcoding.
- Feature additions: introduced pre-approval logic for change requests, added contributing attributes for poverty score, and enhanced JSON templates for DCI response handling.

### New in this build (since 0.0.0-develop.158)
...
<a id="v-0-0-0-develop-158"></a>

## national-social-registry — develop 0.0.0-develop.158 (2026-07-23)

_commit `d0d7d4c` · baseline: release the start · previous build 0.0.0-develop.157_
<!-- build:0.0.0-develop.158 revision:d0d7d4c7b72e0bbf7eea1e6c001de2c50aad36ee ts:1784790238 -->

### Summary

_All changes since release the start:_

- **Major:** Refactor of NSR deployment: transitioned to a thin extension model with a new openg2p-nsr wrapper, aligning dependencies and simplifying Docker images.
- Security enhancements: added CSRF validation for staff-portal-api requests and configured IAM permissions for register metadata API access.
- CI/CD improvements: integrated NSR field-specific tests into CI, updated Helm chart configurations, and removed legacy logging and hardcoding for better maintainability.
- Database updates: refined SQL seed data for households and individuals, improved geo data handling, and added support for new attributes related to poverty scoring.
- Dependency management: updated multiple library versions, fixed issues in metadata, and ensured compatibility with the latest openg2p-registry and other components.
- Feature additions: implemented pre-approval logic for change requests, enhanced JSON templates for DCI response handling, and added new scoring definitions for poverty computation.
- Resource management: addressed OOM issues by removing memory limits in Redis and ensuring dockers run as non-root users for improved security.
- Documentation updates: added README for NSR extension package and moved documentation to Gitbook for better accessibility.

### New in this build (since 0.0.0-develop.157)
